...
class NextTestQuestionView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            question, options = get_next_question_data(
                serializer.validated_data["group_name"]
            )
            response_serializer = AdaptiveTestQuestionSerializer(
                AdaptiveTestQuestionBundle(question, options)
            )
            return Response(
                {"question": response_serializer.data}, status=status.HTTP_200_OK
            )
...
def get_next_question_data(question_group_name):
    try:
        ...
        raise AdaptiveTestError(
            f"Question group with name '{question_group_name}' does not exist."
        )
